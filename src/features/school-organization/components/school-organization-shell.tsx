...
type AccessProfileRecord = {
  id: string;
  userId: string;
  dashboardRole: Exclude<DashboardRole, 'Inherited'>;
...
function AccessEditorSheet({
  ...
  const [dashboardRole, setDashboardRole] = useState<DashboardRole>('Inherited');
  ...
  useEffect(() => {
    ...
    if (rows.length === 1) {
      const row = rows[0];
      setDashboardRole(row.dashboardRole);
      setPermissions(normalizeDashboardPermissions(row.managed ? row.permissions : []));
      return;
    }

    const firstRole = rows[0]?.dashboardRole ?? 'Inherited';
    const allSameRole = rows.every((row) => row.dashboardRole === firstRole);
    ...
    setDashboardRole(allSameRole ? firstRole : 'Custom');
    ...
    try {
      ...
      if (isBulk) {
        await bulkSaveAccessProfiles({
          orgId,
          userIds,
          dashboardRole: dashboardRole as Exclude<DashboardRole, 'Inherited'>,
  ...
  function handleRoleChange(nextRole: DashboardRole) {
    ...
    setPermissions(normalizeDashboardPermissions(permissionSetForRole(nextRole)));
  }
  ...
  function togglePermission(permission: DashboardPermissionKey, checked: boolean) {
    ...
    if (
      dashboardRole !== 'Inherited' &&
      dashboardRole !== 'Custom' &&
      !roleMatchesPermissions(dashboardRole, nextPermissions)
    ) {
      setDashboardRole('Custom');
    }
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className='flex flex-col sm:max-w-2xl'>
        <SheetHeader>
          ...
          <SheetDescription>
            {isBulk
              ? `Apply one permission set to ${rows.length} selected staff members.`
              : 'Choose a dashboard role, then fine-tune module access for this staff member.'}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className='flex-1 space-y-6 overflow-auto pr-1'>
          ...
          <div className='grid gap-3'>
            <div className='space-y-2'>
              <div className='text-sm font-medium'>Dashboard role</div>
              <Select
                value={dashboardRole}
                onValueChange={(value) => handleRoleChange(value as DashboardRole)}
              >
                <SelectTrigger>
                  <SelectValue placeholder='Choose a role' />
                </SelectTrigger>
                <SelectContent>
                  {DASHBOARD_ROLE_OPTIONS.map((role) => (
              ...
              <p className='text-muted-foreground text-xs'>
                Preset roles fill the checkbox list for you. Switch to Custom if this staff member
                needs a mixed permission set.
              </p>
            </div>
  ...
  const rows = useMemo<StaffAccessRow[]>(() => {
    ...
    return members
      .map((membership) => {
        ...
        const storedRole = profile?.dashboardRole ?? '';
                  ...
                  <DataTableToolbar table={table}>
                    <div className='hidden text-xs text-muted-foreground xl:block'>
                      {hasFilters
                        ...
                        : 'Unmanaged staff keep today’s broader dashboard visibility until you assign a Schly permission profile.'}
                    </div>
                  </DataTableToolbar>
                </DataTable>
